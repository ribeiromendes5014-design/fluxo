...
# Supondo que esses arquivos existam e estejam corretos
# from constants_and_css import * # from render_utils import render_global_config, render_custom_header
# from utils import *

# Funções placeholder para garantir que o código não quebre se os imports falharem
...
def livro_caixa():
    ...
    with tab_nova_mov:
        ...
        if tipo == "Entrada":
            ...
            with col_principal_2:
                ...
                termo_busca = st.text_input("Buscar Cliente", value="", key="input_busca_cliente", disabled=edit_mode)
            ...
            with st.expander("➕ Adicionar/Limpar Lista de Produtos (Venda)", expanded=True):
                ...
                with col_prod_add:
                    ...
                    foto_cb_upload_caixa = st.file_uploader("📤 Upload de imagem do código de barras", type=["png", "jpg", "jpeg"], key="cb_upload_caixa")
                    ...
                    if is_recorrente and not edit_mode:
                        ...
                        for i in range(1, num_parcelas_int + 1):
                            ...
                            nova_linha_parcela = {
                                ...
                                "Valor": -valor_parcela_float, "Forma de Pagamento": "Pendente", 
                        ...
                        categoria_final = categoria_selecionada if tipo == "Saída" else loja_selecionada
                        
                        nova_linha_data = {
                            "Data": data_input, "Loja": loja_selecionada, "Cliente": cliente_final,
                            "Valor": valor_armazenado, "Forma de Pagamento": forma_pagamento,
                            "Tipo": tipo, "Produtos Vendidos": produtos_vendidos_json,
                            "Categoria": categoria_final, "Status": status_selecionado, 
                            "Data Pagamento": data_pagamento_final, "RecorrenciaID": "", "TransacaoPaiID": "" 
                        }
    ...
    with tab_mov:
        hoje = date.today()
        primeiro_dia_mes = hoje.replace(day=1)
        ultimo_dia_mes = (primeiro_dia_mes.replace(month=primeiro_dia_mes.month % 12 + 1) - timedelta(days=1)) if primeiro_dia_mes.month != 12 else primeiro_dia_mes.replace(year=primeiro_dia_mes.year+1, month=1) - timedelta(days=1)
        ...
        if not df_pendente_alerta.empty:
            ...
            df_vencidas = df_pendente_alerta.dropna(subset=["Data Pagamento"])[df_pendente_alerta["Data Pagamento"] <= hoje_date]
            if not df_vencidas.empty:
                ...
                num_receber = len(df_vencidas[df_vencidas["Tipo"] == "Entrada"])
                num_pagar = len(df_vencidas[df_vencidas["Tipo"] == "Saída"])
                
                alert_message = "### ⚠️ DÍVIDAS PENDENTES VENCIDAS!"
        ...
        st.subheader(f"📋 Tabela de Movimentações")
        st.dataframe(df_exibicao.style.apply(highlight_value, axis=1), use_container_width=True)
