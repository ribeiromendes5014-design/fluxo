...
def homepage():
    ...
    if produtos_novos.empty:
        ...
        for i, row in produtos_novos.iterrows():
            foto_url = row.get("FotoURL") if row.get("FotoURL") else f"https://placehold.co/400x400/FFC1E3/E91E63?text={str(row.get('Nome','')).replace(' ', '+')}"
            preco_vista = to_float(row.get('PrecoVista', 0))
            descricao = f"R$ {preco_vista:,.2f}" if preco_vista > 0 else "Preço não disponível"
            nome = row.get('Nome','')
            marca = row.get('Marca','')
            qtd = int(row.get('Quantidade', 0)) if pd.notna(row.get('Quantidade', 0)) else 0
            card_html = f'''
                <div class="product-card">
