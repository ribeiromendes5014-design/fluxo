...
import base64
import calendar # <-- NOVO: Adiciona import para manipulação de datas
...
def add_months(d: date, months: int) -> date:
    """
    Adiciona um número específico de meses a uma data, ajustando para 
    o final do mês se necessário (ex: 31/01 + 1 mês -> 28/02).
    """
    month = d.month + months
    year = d.year + (month - 1) // 12
    month = (month - 1) % 12 + 1
    
    # Encontra o último dia do novo mês
    # Usa calendar.monthrange para descobrir quantos dias o novo mês tem
...
def carregar_livro_caixa():
    ...
    # Adiciona RecorrenciaID para a nova funcionalidade, se não existir
    if 'RecorrenciaID' not in df.columns:
        df['RecorrenciaID'] = ''
            
    # Retorna todas as colunas necessárias (padrão + RecorrenciaID)
    cols_to_return = COLUNAS_PADRAO + ["RecorrenciaID"]
    # Garante que só retorna colunas que existem (evita erro se o CSV for muito antigo/vazio)
...
def processar_dataframe(df):
    ...
    """
    # Garante que o df contém todas as colunas esperadas para processamento
    for col in COLUNAS_PADRAO:
        if col not in df.columns:
            df[col] = "" # Adiciona colunas ausentes
            ...
            if tipo_produto == "Produto com variações (grade)":
                ...
                for i in range(int(qtd_variações)):
                    ...
                    variações.append({
                        ...
                        "CodigoBarras": var_codigo_barras # Correção: Usar o valor lido ou digitado no input
                    })
            ...
            if salvar_compra:
                if not nome_produto or valor_total_calculado <= 0 or quantidade <= 0:
                    ...
                    nova_linha = {
                        ...
                        "Valor Total": valor_total_calculado, # Usa o valor calculado (Corrigido)
                        "Cor": cor_selecionada,
                        "FotoURL": foto_url.strip(),
                    }
...
def livro_caixa():
    ...
    # Adiciona a coluna 'RecorrenciaID' se não existir (para ser salva no CSV)
    if 'RecorrenciaID' not in st.session_state.df.columns:
        st.session_state.df['RecorrenciaID'] = ''
    ...
    with st.sidebar:
        ...
        # Desabilitado em modo edição para simplicidade
        tipo = st.radio("Tipo", ["Entrada", "Saída"], index=0 if default_tipo == "Entrada" else 1, key="input_tipo", disabled=edit_mode)
        
        # Variáveis de controle de fluxo para a nova funcionalidade de recorrência
        is_recorrente = False
        status_selecionado = default_status
        data_primeira_parcela = date.today().replace(day=1) + timedelta(days=32) # Sugere próximo mês
        ...
        if tipo == "Entrada":
            ...
            # --- NOVO: OPÇÃO DE DÍVIDA RECORRENTE ---
            st.markdown("#### Opções de Saída")
            
            if not edit_mode:
                # O checkbox de recorrência só aparece para novas movimentações
            ...
            categoria_selecionada = st.selectbox("Categoria de Gasto", 
                                                    ...
                                                    disabled=is_recorrente and not edit_mode) # Desabilita se for recorrência
            ...
            if is_recorrente and not edit_mode:
                # --- FORMULÁRIO DE DÍVIDA RECORRENTE ---
                st.markdown("##### 🧾 Detalhes da Recorrência")
                
                # Nome da Despesa (usado como Cliente/Descrição)
                ...
                # Primeira data de vencimento (Data Pagamento)
                data_primeira_parcela = st.date_input("Data de Vencimento da 1ª Parcela", 
                                                      value=date.today().replace(day=1) + timedelta(days=32), # Sugere o próximo mês
            ...
            else: # Saída Simples (Não Recorrente ou Editando)
                
                # Status para Saída Simples/Edição
                status_selecionado = st.radio(
                    "Status", 
                    ["Realizada", "Pendente"], 
                    index=0 if default_status == "Realizada" else 1, 
        ...
        # --- LÓGICA DE DATA DE PAGAMENTO (Para Pendente Simples/Edição) ---
        data_pagamento_final = None 

        if status_selecionado == "Pendente" and not (is_recorrente and not edit_mode): # Se for pendente simples ou edição de pendente
            ...
            # Tenta usar o valor anterior da sessão
            ...
            data_status_selecionado_previsto = st.radio(
                ...
                key=data_status_key, 
                horizontal=True,
                disabled=edit_mode and default_status == "Pendente" and data_prevista_existe # Impede mudança se já tiver data e estiver editando
            )
            ...
            # Data de pagamento final é a primeira parcela da recorrência
        ...
        with st.form("form_movimentacao_sidebar", clear_on_submit=not edit_mode):
            ...
            # Cliente/Descrição: Usa o nome da recorrência se for o caso, senão o default
            ...
            if enviar:
                ...
                # Verifica se é Saída Simples e categoria é Outro/Diversos
                    ...
                    # --- NOVO: LÓGICA DE GERAÇÃO DE PARCELAS ---
                    ...
                    if is_recorrente and not edit_mode:
                        ...
                        # Gera um ID de recorrência único (hash baseado em inputs)
                        ...
                        for i in range(1, num_parcelas_int + 1):
                            ...
                            # Calcula a data de vencimento da parcela, avançando "i - 1" meses
                            data_vencimento_parcela = add_months(data_primeira_parcela, i - 1)
                            
                            nova_linha_parcela = {
                                "Data": data_input, # Data de lançamento
                                "Loja": loja_selecionada, 
                                "Cliente": f"{nome_despesa_recorrente} (Parc. {i}/{num_parcelas_int})",
                                "Valor": -valor_parcela_float, # Saída é sempre negativa
                                "Forma de Pagamento": "Pendente", 
                                "Tipo": "Saída",
                                "Produtos Vendidos": "",
                                "Categoria": categoria_selecionada,
                                "Status": "Pendente",
                                "Data Pagamento": data_vencimento_parcela, # Data de vencimento da parcela
                        ...
                        # Lógica para Saída Simples ou Entrada
                        nova_linha_data = {
                            "Data": data_input,
                            "Loja": loja_selecionada, 
                            "Cliente": cliente,
            ...
            if df_para_mostrar.empty:
                ...
                if original_idx_selecionado is not None and movimentacao_selecionada_str != "Selecione uma movimentação...":
                    ...
                    if col_op_2.button(f"🗑️ Excluir: {item_selecionado_str}", key=f"del_mov_{original_idx_selecionado}", use_container_width=True, type="primary"):
                        ...
                        # Exclui a linha do DF original usando o original_index
    ...
    with tab_rel:
        ...
        df_anual = df_anual[df_anual['Ano'] > 0] # Remove anos inválidos
        ...
        if not df_anual.empty:
            ...
            fig_anual = px.bar(
                ...
                x='Ano', 
                y=['Entradas', 'Saídas'], 
                title="Entradas vs. Saídas por Ano",
            ...
            st.info("Dados insuficientes para gerar relatório anual.")

        st.markdown("---")
        
        # --- LÓGICA DE DÍVIDAS PENDENTES (A PAGAR / A RECEBER) ---
        ...
        if df_pendentes.empty:
            ...
            # Formata a coluna "Data Pagamento" para exibição e ordenação (garantindo que seja date)
            df_pendentes["Data Pagamento"] = pd.to_datetime(df_pendentes["Data Pagamento"], errors='coerce').dt.date
            
            # Ordena por Data Pagamento (Data Prevista)
            df_pendentes_ordenado = df_pendentes.sort_values(
                by=["Data Pagamento", "Tipo", "Data"], 
            ...
            def highlight_pendentes(row):
                """Destaque para vencidas e próximas do vencimento."""
                ...
                # Não vencida (Transparente)
                return ['' for col in row.index]

            # Campos para seleção de conclusão de dívida
            with st.form("form_concluir_divida"):
                ...
                with col_c2:
                    # Permite selecionar a forma de pagamento (se for a Pagar)
                    forma_pagt_concluir = st.selectbox("Forma de Pagamento (Realizada)", 
                                                        FORMAS_PAGAMENTO, 
                                                        key="forma_pagt_concluir")
                ...
                if concluir and original_idx_concluir is not None:
                    ...
                    # Encontra a linha no DF original da sessão
                    ...
                    st.session_state.df.loc[idx_original, 'Data'] = data_conclusao # Transação passa para a data real de conclusão
                    st.session_state.df.loc[idx_original, 'Data Pagamento'] = data_conclusao # Data de pagamento
                    
                    # 2. Atualiza Forma de Pagamento (Se for Saída, só para garantir que o 'Pendente' seja substituído)
                    st.session_state.df.loc[idx_original, 'Forma de Pagamento'] = forma_pagt_concluir
                    
                    # 3. Lógica de estoque (Se for Entrada - A Receber)
                    if row_data["Tipo"] == "Entrada" and row_data["Produtos Vendidos"]:
                        # Debita o estoque agora que a venda foi realizada
                        try:
                            ...
                            if salvar_produtos_no_github(st.session_state.produtos, f"Débito de estoque por conclusão de venda {row_data['Cliente']}"):
                                inicializar_produtos.clear()

                        except:
                            st.warning("⚠️ Venda concluída, mas falha no débito do estoque (JSON inválido).")
                    ...
                    st.warning("Selecione uma dívida válida para concluir.")

            st.markdown("---")

            # Exibe a Tabela de Pendentes
            st.markdown("##### Tabela Detalhada de Dívidas Pendentes")
            
            df_para_mostrar_pendentes = df_pendentes_ordenado.copy()
            
            # Adiciona a coluna de Atraso formatada
            ...
            # Oculta colunas auxiliares (Cor_Valor e Dias Até/Atraso)
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Data": st.column_config.DateColumn("Data Lançamento"),
                    "Data Pagamento": st.column_config.DateColumn("Data Vencimento/Prevista", format="DD/MM/YYYY"),
                    "Status Vencimento": st.column_config.TextColumn("Vencimento"),
                    "Cliente": st.column_config.TextColumn("Descrição"),
                    "RecorrenciaID": st.column_config.TextColumn("ID Recorrência", help="Identificador único para parcelas recorrentes")
                },
                column_order=['ID Visível', 'Status Vencimento', 'Tipo', 'Loja', 'Cliente', 'Categoria', 'Valor', 'Data Pagamento', 'Data', 'RecorrenciaID'],
                hide_index=True,
                selection_mode='single-row',
                key='pendentes_table_styled'
            )
...
PAGINAS = {
    ...
    "Gestão de Produtos (Estoque)": gestao_produtos,
    "Histórico de Compras (Insumos)": historico_compras
}

if "pagina_atual" not in st.session_state:
    st.session_state.pagina_atual = "Livro Caixa"

# --- SIDEBAR: Seleção de Página ---
with st.sidebar:
    st.image("https://placehold.co/150x50/3498db/ffffff?text=Logo")
    st.markdown("### Navegação")
    # Usa um seletor para mudar a página (o radio button causa um re-run imediato)
    pagina_selecionada = st.radio(
        "Selecione a página:", 
        options=list(PAGINAS.keys()),
        index=list(PAGINAS.keys()).index(st.session_state.pagina_atual),
        key="main_page_selector"
    )
    st.session_state.pagina_atual = pagina_selecionada
    
    st.markdown("---")
    # O formulário de Movimentação aparece APENAS na página Livro Caixa e é definido dentro da função `livro_caixa()`
